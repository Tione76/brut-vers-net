...
interface IncreaseFormStepProps {
  step: number;
  title: string;
  hint?: string;
  essential?: boolean;
...
export function IncreaseFormStep({ step, title, hint, essential, children }: IncreaseFormStepProps) {
  return (
    <li
      className={`increase-calc__step${essential ? " increase-calc__step--essential" : ""}`}
      aria-labelledby={`increase-step-${step}-title`}
    >
      ...
      <div className="increase-calc__step-body">
        <h3 id={`increase-step-${step}-title`} className="increase-calc__step-title">
          {title}
        </h3>
