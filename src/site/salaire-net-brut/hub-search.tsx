...
import { netToGrossHubFicheDomId } from "./hub";
import {
  findPublishedNetNearest,
  parseNetSearchInput,
} from "./index-table";

interface NetToGrossHubAmountSearchProps {
 ...
 */
export function NetToGrossHubAmountSearch({ amounts }: NetToGrossHubAmountSearchProps) {
  const titleId = useId();
  const inputId = useId();
  ...
  function highlightAmount(amount: number) {
    clearHighlight();
    const target = document.getElementById(netToGrossHubFicheDomId(amount));
    if (!target) {
      return;
    }
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus(
      `Montant le plus proche : ${label} (les fiches avancent par pas de 100 €).`,
    );
  }
