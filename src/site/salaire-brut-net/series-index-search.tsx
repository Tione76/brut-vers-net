...
interface GrossToNetIndexTableSearchProps {
  amounts: readonly number[];
}

/**
 * Navigation rapide dans le tableau Index (aucun recalcul de net).
 */
export function GrossToNetIndexTableSearch({ amounts }: GrossToNetIndexTableSearchProps) {
  const titleId = useId();
  const inputId = useId();
  ...
  function highlightRow(amount: number) {
    clearHighlight();
    const row = document.getElementById(grossToNetIndexRowId(amount));
    if (!row) {
      return;
    }
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus(
      `Montant le plus proche : ${label} (les valeurs du tableau avancent par pas de 50 €).`,
    );
  }
