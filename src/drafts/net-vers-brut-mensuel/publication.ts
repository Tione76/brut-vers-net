/**
 * État de la série Net → Brut mensuel après vague 5 des intermédiaires (pas de 10 €).
 *
 * - Publiés : 271 (46 centaines + 225 intermédiaires 1 510 → 3 990)
 * - Brouillons : 180 (4 010 → 5 990 hors centaines)
 *
...
const EXPECTED_PUBLISHED_COUNT = 271;
const EXPECTED_DRAFT_COUNT = 180;
...
export function assertTenEuroIntermediatesPrepared(): void {
  ...
  if (DRAFT_NET_TO_GROSS_AMOUNTS[0] !== 4010 || DRAFT_NET_TO_GROSS_AMOUNTS[EXPECTED_DRAFT_COUNT - 1] !== 5990) {
    throw new Error("Les brouillons restants doivent aller de 4 010 € à 5 990 €.");
  }
  ...
  for (const amount of [1510, 2990, 3010, 3490, 3510, 3750, 3990] as const) {
    if (!published.has(amount)) {
