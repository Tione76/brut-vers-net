...
export const alt = "Tableau de conversion salaire net mensuel en brut";
export const size = NET_TO_GROSS_OG_SIZE;
export const contentType = NET_TO_GROSS_OG_CONTENT_TYPE;

export default async function NetToGrossIndexOpenGraphImage() {
  try {
    const model = await buildNetToGrossStaticOgRenderModel({
      headline: "TABLEAU NET → BRUT",
      question: "Conversion mensuelle",
    });
    ...
    return new ImageResponse(
      (
        ...
        >
          <div style={{ display: "flex", fontSize: 64, fontWeight: 700 }}>TABLEAU NET → BRUT</div>
          <div style={{ display: "flex", fontSize: 36, marginTop: 16 }}>Conversion mensuelle</div>
